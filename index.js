...
case 'timer':
   ...
   case '(nmrd)':
		if (args.length < 1) return reply ('(AQUI ESTÃO OS NÚMEROS DOS PROPRIETÁRIOS:
NABUTO LS: wa.me/556993733829)')
		break
   
		case '(cmmd)':
		if (args.length < 1) return reply ('VOCÊ TERA QUE ESCREVER ASSIM: *daftar (aqui você coloca seu nome) (aqui sua idade
exemplo: *daftar ninem|14')
		break
                case 'bahasa':
  ...
  case 'belledecalcinha':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://images.app.goo.gl/kLEfPKY2jw2kMTWPA`)					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'essa aí é gostosa visse, vou até tomar meu "banho" ali'})
					break
